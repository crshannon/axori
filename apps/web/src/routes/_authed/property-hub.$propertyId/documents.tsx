...
import { Upload, Search, Grid, List, FolderOpen, Loader2 } from 'lucide-react'
...
function DocumentsPage() {
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <h3 className="text-xl font-black tracking-tight mb-2">No documents yet</h3>
      <p className="text-sm text-slate-500 dark:text-white/60 text-center max-w-sm mb-6">
        Upload your first document to start organizing your property paperwork for tax preparation.
      </p>
      {canEdit && (
        <button
    ...
    <div className="p-8 w-full grid grid-cols-1 lg:grid-cols-4 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-2 text-sm">
              ...
              {stats.totalSizeBytes > 0 && (
                <div className="flex justify-between pt-2 border-t border-slate-200 dark:border-white/10">
                  <span className="text-slate-600 dark:text-white/70">Storage</span>
      ...
      <div className={`${cardClass} lg:col-span-3`}>
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          ...
          {/* Clear Filters */}
          {(selectedType || selectedYear || selectedStatus || searchQuery) && (
            <button
              onClick={() => {
                setSelectedType('')
                setSelectedYear('')
